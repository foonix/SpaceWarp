...
internal class ModListController : MonoBehaviour
{
    ...
    internal void AddMainMenuItem()
    {
        ...
        if (UI.Instance.ConfigShowMainMenuWarningForErroredMods.Value &&
            PluginList.AllErroredPlugins.Count > 0)
        {
            term = "SpaceWarp/Mods/Errored";
        }
        else if (UI.Instance.ConfigShowMainMenuWarningForOutdatedMods.Value &&
    ...
    private void SetupButtons()
    {
        ...
        _applyChangesButton.RegisterCallback<ClickEvent>(_ =>
        {
            ...
            UI.Instance.ModuleLogger.LogDebug($"Restarter path: {restarterPath}");
    ...
    {
        var element = UI.ListEntry.Instantiate();
        ...
        List<string> disabledDependencies)
    {
        ...
        {
            var dependencyElement = UI.DependencyEntry.Instantiate();
